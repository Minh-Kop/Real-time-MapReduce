...
if __name__ == "__main__":
    ...
    user_file_path = "./input/users_test.txt"

    hdfs_input_file_path = f"{HADOOP_PATH}/input/{get_txt_filename(input_file_path)}"
    hdfs_user_file_path = f"{HADOOP_PATH}/input/{get_txt_filename(user_file_path)}"

    ## Put input files to HDFS
    put_files_to_hdfs(input_file_path, hdfs_input_file_path)
    put_files_to_hdfs(user_file_path, hdfs_user_file_path)

    ## Run Jaccard
    run_jaccard(
        hdfs_input_file_path,
        hdfs_user_file_path,
        output_file_path="hadoop_output/jaccard.txt",
    )
