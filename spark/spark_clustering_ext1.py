...
spark_config = (
    SparkConf().setAppName("spark_clustering").set("spark.driver.memory", "4g")
)
...
def write_to_file(df, file_name):
    df.coalesce(1).write.mode("overwrite").csv(
        "file://" + os.path.abspath(file_name), header=True
    )
...
temp_df = temp_df.withColumn(
    "rating", when(col("rating").isNull(), col("avg(rating)")).otherwise(col("rating"))
)
...
temp_df = temp_df.orderBy(col("sum(temp)").desc()).limit(
    number_of_clusters * multiplier
)
temp_df = (
    temp_df.join(fill_matrix_df, on="user")
    .drop(*["sum(temp)", "categories"])
    .orderBy(["user", "item"])
)
...
for i in range(number_of_clusters - 1):
    ...
    temp_df = array_df.crossJoin(
        init_centroids_df.limit(1).select(
    ...
    temp_df = (
        temp_df.withColumn(
            "distance", euclidean_distance_udf(col("ratings"), col("ratings_"))
        )
